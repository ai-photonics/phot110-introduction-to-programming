...
### INTERMEZZO: Methods versus Functions

Notice that we use the dot-notation: `the_list.append(the_element)`. We already saw this notation in a different context: when using for example the square root function `sqrt()` of the `math` module:

```{python}
import math
a = math.sqrt(2)  # This is a function
print(f"The square root of 2 = {a}")
```

However, here this notation is not used to use a function of an imported package/module, here we use it to call a method on an object: we use the method `append()` on the list (the object). 
