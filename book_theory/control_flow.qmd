...
The for loop can also be used to repeat a process a known number of times. For this we replace the list by the **`range`** function, which can represent a "list" of numbers. The **`range`** function returns actually a sequence type (like `list`) for integer numbers, we can construct it as follows: `range(start, stop_exclusive, step)`, where:
...
The `range()` does not give a list, but a "range" object: this object is more memory efficient, it only generates the integer numbers when asked for (imagine you want to do billions of iterations). We can cast a range object into a list though:
...
In science and engineering we often use numerical methods to analyse systems. As a practical example of using a while loop, let us calculate the trajectory $(x(t), y(t))$ of a ball numerically. The ball has an initial position $(x(0), y(0)) = (0, 1.20)$ (in meters) and velocity $v_0 = 6$ m/s thrown under an angle of 37 degrees, see the figure below. 
...
The numerical algorithm adapts the ball's velocity at small discrete time steps, and the resulting coordinates form an approximate trajectory. The code uses the while loop to calculate subsequent coordinates until the ball drops on the ground (the stopping criterium). See the code below.
